...
function plot_sset(ss::SSet, points::Vector,
  ...
  lengthscale = 0.8  # Validate inputs
  dim = length(points[1])
  length(points) == nparts(ss, :V) || error("# of points")
  if dim == 2
    points = [(p1, p2, 0.0) for (p1, p2) in points]
  elseif dim != 3
    error("dim $dim")
  end
  tri_colors = tri_colors[1:nparts(ss, :T)]

  s = EmbeddedDeltaSet2D{Bool,Point{3,Float64}}()  # convert SSet to EmbeddedDeltaSet2D
  ...
  m = GeometryBasics.Mesh(s)  # split mesh into component triangles
  x = faces(m)
...
six_coords = vcat(quad_coords, [(-1.0, 1.0, 0.0), (-1.0, 0.0, 0.0),])
